...
class Spherometer:
    ...
    def expr_df(self) -> sy.Symbol:
        ...
        drsda = r_s/a - (a*b*c/8)*((-2*k**3 + (b+c)*k**2 + (a*b + a*c)*k - a*b*c/2)/q)
        drsdb = r_s/b - (a*b*c/8)*((-2*k**3 + (a+c)*k**2 + (b*a + b*c)*k - a*b*c/2)/q)
        drsdc = r_s/c - (a*b*c/8)*((-2*k**3 + (a+b)*k**2 + (c*b + c*a)*k - a*b*c/2)/q)
    ...
    def record(self, dial_meas=None, sagitta=None, file="record.csv"):
        ...
        data = {
            ...
            "s": [sagitta],
            "f": [sy.lambdify([a, b, c, d_b, s], self.expr_f())(
                self.a,
                self.b,
                self.c,
                self.d_b,
            ...
            "df": [sy.lambdify([a, b, c, d_b, s, ds, dx], self.expr_df())(
                self.a,
                self.b,
                self.c,
                self.d_b,
        ...
        if pathlib.Path(file).exists():
            df = pd.concat(
                [pd.read_csv(file), pd.DataFrame(data)]
            ).set_index('datetime')
        else:
    ...
    def save(self, file="spherometer.toml"):
        path = pathlib.Path(file)

        if path.exists():
            raise ValueError(f"Path to {file} already exists!")

        with open(path, 'wb') as f:
            tomli_w.dump(vars(self), f)

    @classmethod
    def load(cls, file="spherometer.toml"):
        with open(pathlib.Path(file), 'rb') as f:
            config = tomli.load(f)
